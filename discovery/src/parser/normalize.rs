...
pub fn normalize_help_output(raw: &str) -> String {
    ...
    static ANSI_RE: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"\x1b\[[0-9;?]*[ -/]*[@-~]").expect("static regex must compile")
    });
    static OVERSTRIKE_RE: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r".\x08").expect("static regex must compile"));
    ...
    for line in replaced.lines() {
        ...
        let is_wrapped_continuation = (line.starts_with(' ') || line.starts_with('\t'))
            ...
            && normalized.last().is_some_and(|prev| {
                ...
                let starts_new_compact_option_row =
                    HelpParser::looks_like_compact_option_row(trimmed_start);
                let starts_new_symbolic_option_row =
                    HelpParser::looks_like_symbolic_option_row(trimmed_start);
                ...
                (prev_is_flag || prev_is_two_column_subcommand)
                    ...
                    && !starts_new_flag_row
                    && !starts_new_compact_option_row
                    && !starts_new_symbolic_option_row
            });
