...
fn test_parse_kubectl_fixture_extracts_flags_and_subcommands() {
    ...
    assert!(schema.find_subcommand("get").is_some(), "missing 'get' subcommand");
    assert!(schema.find_subcommand("edit").is_some(), "missing 'edit' subcommand");
...
fn test_parse_coreutils_and_complex_fixtures() {
    ...
    assert!(cp_schema.find_global_flag("-r").is_some(), "missing '-r' flag");
